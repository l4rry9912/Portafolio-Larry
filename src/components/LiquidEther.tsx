...
/** Máximo lado interno del fluido (px simulados); evita coste O(n²) en monitores grandes. */
const MAX_FLUID_SIM_EDGE = 400;
...
function fluidQualityForViewportWidth(cssWidth: number): FluidQuality {
  if (cssWidth < 768) {
    return { maxRes: 0.34, poissonCap: 18, viscousCap: 18, autoMul: 0.68 };
  }
  if (cssWidth < 1024) {
    return { maxRes: 0.36, poissonCap: 18, viscousCap: 18, autoMul: 0.8 };
  }
  if (cssWidth < 1536) {
    return { maxRes: 0.34, poissonCap: 16, viscousCap: 16, autoMul: 0.88 };
  }
  return { maxRes: 0.3, poissonCap: 14, viscousCap: 14, autoMul: 0.9 };
}

function pixelRatioForContainerCssWidth(cssWidth: number): number {
  const dpr = typeof window !== 'undefined' ? window.devicePixelRatio || 1 : 1;
  if (cssWidth < 768) return Math.min(dpr, 1);
  if (cssWidth > 1440) return Math.min(dpr, 1);
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      ...
      timeout = window.setTimeout(() => setViewportWidth(window.innerWidth), 120);
    };
  ...
  const fluidQ = fluidQualityForViewportWidth(viewportWidth);
  const resolutionSim = Math.min(resolution, fluidQ.maxRes);
  const iterationsPoissonSim = Math.min(iterationsPoisson, fluidQ.poissonCap);
  const iterationsViscousSim = Math.min(iterationsViscous, fluidQ.viscousCap);
  const autoSpeedSim = autoSpeed * fluidQ.autoMul;
  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      init(container: HTMLElement) {
        ...
        this.resize();
        this.pixelRatio = pixelRatioForContainerCssWidth(this.width);
      ...
      refreshPixelRatio() {
        ...
        const next = pixelRatioForContainerCssWidth(this.width);
...
`;
    ...
    class Simulation {
      ...
      calcSize() {
        ...
        if (longEdge > MAX_FLUID_SIM_EDGE) {
          const s = MAX_FLUID_SIM_EDGE / longEdge;
    ...
    const applyOptionsFromProps = () => {
      ...
      Object.assign(sim.options, {
        ...
        isViscous,
        viscous,
        iterations_viscous: iterationsViscousSim,
        iterations_poisson: iterationsPoissonSim,
        dt,
        BFECC,
        resolution: resolutionSim,
        isBounce
      });
      if (resolutionSim !== prevRes) sim.resize();
    };
  ...
  useEffect(() => {
    ...
    if (webgl.autoDriver) {
      ...
      webgl.autoDriver.speed = autoSpeedSim;
